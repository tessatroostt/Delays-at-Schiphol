...
if sidebar_keuze == 'Flights vs Covid':
  ...
  with col1:
    fig = px.line(flights, x="Date", y=['Europe', 'Intercontinental',"Total"], title='Amount of flights at Schiphol Airport Amsterdam over the years 2018-2021',
             color_discrete_map = {'Europe': 'rgb(220, 176, 242)', 'Intercontinental': 'rgb(158, 185,243)', 'Total': 'rgb(254, 136, 177)'}).update_layout(width = 400, yaxis_title = 'Number of flights', legend_title = 'Region' )
    st.write(fig)
  
  with col2:
    fig = px.bar(total, x="Year", y="Flights", color = 'Region',barmode = 'group', color_discrete_map = {'Europe': 'rgb(220, 176, 242)', 'Intercontinental': 'rgb(158, 185,243)', 'Total': 'rgb(254, 136, 177)'}).update_layout(xaxis=dict(tickformat="%Y"), width = 400, title = 'Amount of flights at Schiphol Airport Amsterdam 2018-2021', yaxis_title = 'Amount of flights')
    st.write(fig)
...
if sidebar_keuze == 'Weather analysis at Schiphol':
  ...
  if linechart_opties == 'Windspeed':
    ...
    fig4.update_layout(title_text="Weather delays vs. Windspeed", width = 1500, height = 700)
  ...
  if linechart_opties == 'Temperature':
    ...
    fig4.update_layout(title_text="Weather delays vs. Temperature", width = 1500, height = 700)
    fig4.update_xaxes(title_text="Date")
    fig4.update_yaxes(title_text="Delay in minutes", secondary_y=False)
  ...
  if linechart_opties == 'Precipation':
    ...
    fig4.update_layout(title_text="Weather delays vs. Precipation", width = 1500, height = 700)
    fig4.update_xaxes(title_text="Date")
    fig4.update_yaxes(title_text="Delay in minutes", secondary_y=False)
  ...
  if linechart_opties == 'Maximum visibility':
    ...
    fig4.update_layout(title_text="Weather delays vs. maximum visibility", width = 1500, height = 700)
    fig4.update_xaxes(title_text="Date")
    fig4.update_yaxes(title_text="Delay in minutes", secondary_y=False)
